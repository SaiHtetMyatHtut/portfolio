...
class Student_DB:
    ...
    def remove_student(self, sid):
        con = sql.connect(self.db_name)
        cur = con.cursor()
        cur.execute(f"delete from {self.table_name} where sid = '{sid}'")
        con.commit()
        print()
        return "Successfully deleted"

    def update_student(self, sid, *args):
        con = sql.connect(self.db_name)
        cur = con.cursor()
        if not args:
            ...
            if not data:
                ...
                cur.execute(f"update {self.table_name} set name = '{args}' where sid = '{sid}'")
